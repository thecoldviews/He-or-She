function [segments, fs] = detectVoiced(wavFile,fs,t)
...
Eor = ShortTimeEnergy(x, floor(win*fs), floor(step*fs));
Cor = SpectralCentroid(x, floor(win*fs), floor(step*fs), fs);
...
% % for un-voiced segments
% Limits_uv = [];
% if(Limits(1,1) ~= 1)
%     Limits_uv = [Limits_uv; 1 Limits(1,1)];
% end
% for ii = size(Limits,1)-1
%     Limits_uv = [Limits_uv; Limits(i+1,1)-Limits(i,2)];
% end
% if(Limits(end,2) ~= length(wavFile))
%     Limits_uv = [Limits_uv; Limits(end,2) length(wavFile)];
% end
% segments_uv = {};
% for (i=1:size(Limits_uv,1))
%     segments_uv{end+1} = x(Limits_uv(i,1):Limits_uv(i,2)); 
% end
